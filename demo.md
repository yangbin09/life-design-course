# 人生设计课 (Designing Your Life) - 交互式原型

> “你不需要知道你要去哪里，你只需要知道你要往哪个方向走。” —— Bill Burnett & Dave Evans

这是一个基于《斯坦福人生设计课》核心概念设计的交互式 Web 页面原型。旨在通过互动体验，帮助用户理解和实践人生设计的思维模式。

## 核心功能模块

1.  **设计思维卡片 (Mindsets)**
    *   展示 5 种核心思维：好奇心、试错、重构、觉察、通力合作。
    *   *交互点*：点击翻转卡片，查看具体的“今日行动建议”。

2.  **人生仪表盘 (The Dashboard)**
    *   评估当前的健康、工作、娱乐、爱 (HWPL) 状态。
    *   *交互点*：拖动滑块调整数值，右侧实时显示“诊断建议”。（例如：当某项过低时，提示从微小改变开始）。

3.  **寻路指南针 (Wayfinding)**
    *   帮助用户校准方向。
    *   *交互点*：提供反思问题清单，引导用户思考工作观与人生观的一致性。

4.  **好时光日志 (Good Time Journal)**
    *   记录能量 (Energy) 和 专注度 (Engagement)。
    *   *交互点*：一个迷你输入框，用户可以输入活动名称并打分，动态添加到下方的列表中，直观看到哪些活动给自己充电/耗电。

5.  **奥德赛计划 (Odyssey Plans)**
    *   设计三个版本的未来（当前路径、备选路径、狂野梦想）。
    *   *交互点*：
        *   左侧切换三个计划的 Tab。
        *   右侧展示该计划的详细描述。
        *   **重点交互**：下方的“仪表盘评估”，用户可以拖动滑块，对自己设计的这个人生版本进行资源、喜爱度、信心、一致性的打分。

6.  **解决问题的策略**
    *   重力问题 (Gravity Problems) vs 原型设计 (Prototyping)。
    *   展示如何区分无法解决的事实和可以设计的问题。

## 技术实现 (React Component)

这个原型使用 React + Tailwind CSS 构建。它是一个单文件组件，可以直接复制到任何 React 项目中使用（需安装 `lucide-react` 图标库）。
...
export default DYLPage;  这是前端代码 帮我在当前文件夹创建 合理目录 并且渲染出来
